...
CREATE TABLE products (
    ...
    CONSTRAINT chk_products_retail_price CHECK (retail_price >= 0),
    CONSTRAINT chk_products_wholesale_price CHECK (wholesale_price >= 0),
...
CREATE TABLE store_settings (
    ...
    printer_port INTEGER NOT NULL DEFAULT 8008,
    printer_device_id VARCHAR(120) NOT NULL DEFAULT 'local_printer',
    printer_use_ssl BOOLEAN NOT NULL DEFAULT FALSE,
    printer_buffer BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_store_settings_single_row CHECK (setting_id = 1),
    CONSTRAINT chk_store_settings_tax_rate CHECK (tax_rate >= 0),
    CONSTRAINT chk_store_settings_printer_port CHECK (printer_port BETWEEN 1 AND 65535)
);
